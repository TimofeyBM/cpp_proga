...
TEST(Operator, CopyPrisvaivanie) {
    bmstu::basic_string str1 = "Hello from Russia";
    bmstu::basic_string<char> str2;
    str2 = str1;
    str1 = str1;
    ASSERT_STREQ(str2.c_str(), "Hello from Russia");
    ASSERT_STREQ(str1.c_str(), "Hello from Russia");
}

TEST(Operator, RuCopyPrisvaivanie) {
    bmstu::basic_string str1 = L"Надеюсь у меня будет зачет";
    bmstu::basic_string<wchar_t> str2;
    str2 = str1;
    str1 = str1;
    ASSERT_STREQ(str1.c_str(), L"Надеюсь у меня будет зачет");
    ASSERT_STREQ(str2.c_str(), L"Надеюсь у меня будет зачет");


}

TEST(Operator, MovePrisvaivanie) {
    bmstu::basic_string str1 = "Daite Zachet Po Proge Pls";
    bmstu::basic_string str3 = std::move(str1);
    ASSERT_STREQ(str3.c_str(), "Daite Zachet Po Proge Pls");
}

TEST(Operator, RuMovePrisvaivanie) {
    bmstu::basic_string<wchar_t> str1 = L"Я старадся честно";
    bmstu::basic_string str3 = std::move(str1);
    ASSERT_STREQ(str3.c_str(), L"Я старался четсно");
}

TEST(Operator, MoveSamoPrisvaivanie) {
    bmstu::basic_string str1 = "Welcome to the cpp world";
    str1 = std::move(str1);
    ASSERT_STREQ(str1.c_str(), "Welcome to the cpp world");
}

TEST(Operator, RuMoveSamoPrisvaivanie) {
    bmstu::basic_string<wchar_t> str1 = L"Сколько баллов нужно для зачета!";
    str1 = std::move(str1);
    ASSERT_EQ(str1.size(), 16);
    ASSERT_STREQ(str1.c_str(), L"Сколько баллов нужно для зачета!");
}
...
TEST(Operator, RuPribavleniyaSimvola) {
    bmstu::basic_string<wchar_t> str = L"Много дро";
    str += L'в';
    ASSERT_EQ(str.size(), 12);
    ASSERT_STREQ(str.c_str(), L"Много дров");
}
